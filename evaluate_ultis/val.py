...
def save_one_json(predn, jdict, path, class_map):
    ...
    image_name = int(path.stem) if path.stem.isnumeric() else path.stem
    image_name +=".jpg"
    data_path = "/content/drive/MyDrive/MachineLearningProject/Data/DETR_data/test/_annotations.coco.json"
    with open(data_path, "r") as file:
    # Read the entire content of the file
        data = json.load(file)
    img_id = int([item for item in data['images'] if item['file_name'] == image_name][0]['id'])
...
def run(
    ...
    if save_json and len(jdict):
        ...
        anno_json = str("/content/drive/MyDrive/MachineLearningProject/Data/DETR_data/test/_annotations.coco.json")  # annotations json
        ...
        try:  # https://github.com/cocodataset/cocoapi/blob/master/PythonAPI/pycocoEvalDemo.ipynb
            ...
            print(imgIds)
            print(pred.getImgIds())
            eval.params.imgIds = imgIds
            eval.evaluate()
            eval.accumulate()
            eval.summarize()  # update results (mAP@0.5:0.95, mAP@0.5)
